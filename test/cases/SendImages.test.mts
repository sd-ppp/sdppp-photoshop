...
describe('SendImages', () => {
    ...
    it('should send single image successfully', async () => {
        ...
        // Convert raw buffer to PNG using Jimp
        const jimpImage = await Jimp.read(Buffer.from(imageBlob.buffer));
    ...
    it('should throw error when document not found', async () => {
        const emptyBuffer = new Uint8Array(512 * 512 * 4);
        const jimpImage = await Jimp.read(Buffer.from(emptyBuffer.buffer));
        const pngBuffer = await jimpImage.getBuffer('image/png');
        const pngBlob = new Blob([pngBuffer], { type: 'image/png' });
    ...
    it('should handle multiple images', async () => {
        ...
        const emptyBuffer1 = new Uint8Array(100 * 100 * 4);
        const emptyBuffer2 = new Uint8Array(100 * 100 * 4);
        const jimpImage1 = await Jimp.read(Buffer.from(emptyBuffer1.buffer));
        const jimpImage2 = await Jimp.read(Buffer.from(emptyBuffer2.buffer));
        const pngBuffer1 = await jimpImage1.getBuffer('image/png');
        const pngBuffer2 = await jimpImage2.getBuffer('image/png');
        const pngBlob1 = new Blob([pngBuffer1], { type: 'image/png' });
        const pngBlob2 = new Blob([pngBuffer2], { type: 'image/png' });
